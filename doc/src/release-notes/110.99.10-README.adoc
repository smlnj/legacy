...
* The "required privileges" message is no longer printed when the CM verbose flag is
  set to `false` (e.g., by using the command-line option `-Ccm.verbose=false`)
...
| n.a.
| [.bugid]#375#
| {pull-base}/375[Remove Linux version check from `_arch-n-opsys`]
| n.a.
//| [.bugid]#@ID#
