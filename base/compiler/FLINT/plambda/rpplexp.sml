...
fun ppLexp (pd:int) ppstrm (l: lexp): unit =
	...
	val ppList' : {pp:PP.stream -> 'a -> unit, sep: string} -> 'a list -> unit =
        ...
        fun ppl (VAR v) = pps (LV.toString v)
          | ppl (INT i) = ppi i
        ...
        | ppl (FN(v,t,l)) =
            (openHOVBox 3; pps "FN(";
              pps(LV.toString v); pps ":"; br1 0; prLty' t; pps ",";
              if complex l then
...
*)
        ...
        | ppl (LET(v, r, l)) =
            (openHVBox 2;
              openHOVBox 4;
               pps (LV.toString v); br1 0; pps "="; br1 0; ppl r;
              closeBox();
              newline();
              ppl l;
             closeBox())
  ...
  let fun last (DA.LVAR x) = x
      ...
      fun find le =
        case le
          of VAR w =>
               if (v=w)
               then (pps("VAR " ^ LV.toString v ^ " is free in <lexp>\n");())
               else ()
