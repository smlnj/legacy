...
val bogusMatchResult =  (* the record form *)
      {resDec = A.STRdec [], resStr = M.ERRORstr, resExp = M.CONSTstr M.bogusStrEntity}
val bogusMatchResult1 =  (* the tuple form *)
      (A.STRdec nil, M.ERRORstr, M.CONSTstr M.bogusStrEntity)
 ...
 *                  region * EU.compInfo
 *                   -> (A.dec * M.Structure * M.strExp) option
 *
...
let
             ...
             | STRspec{sign=thisSpecSig as SIG sg, entVar, def, ...} =>
                (let val thisElements = #elements sg
                     ...
                     val (thinDec, thinStr, strExp) =
                         (case matchStr1(thisSpecSig, strStr, sym, tdepth, entEnv, epath',
					 rpath', statenv, region, compInfo)
			    of SOME x => x
			     | NONE => bogusMatchResult1)
  ...
  fun matchIt entEnv =
    let val _ = debugmsg ">>matchIt"
     ...
     in if succeed then (* the signature elements were matched *)
          let val resultEntEnv = EE.mark(mkStamp, resultEntEnv)
           ...
           in SOME (resDec, resStr, resExp)
          end
        else (err EM.COMPLAIN "matchStr (matchStr1/matchIt): signature match failure";
	      NONE)  (* return NONE, indicating signature match failure *)
    end

in

   (* FLINT: we should not do such short-cut matching in the case where the
    * signature contains functor components, because we need to
    * recalculuate the tycpath information for functor components.
    * But completely eliminating shortcut matching is a bit too expensive (still in 2023?),
    * so we include a fctflag in the signature to indicate whether it
    * contains functor components. fctflag disables shortcut matching if
    * it is true, that is, if the signature contains functor components. *)

    if (ST.eq(sigStamp, strSigStamp)) andalso closed andalso (not fctflag)
    then SOME (A.SEQdec [], str, M.VARstr (rev epath))    (* result of short-cut matching *)
...
and matchStr {sign, str, strExp, evOp, tdepth, entEnv, rpath, statenv, region,
    ...
    let val uncoerced = case evOp of SOME x => x | NONE => mkStamp()
	val _ = debugmsg ">>matchStr"
     in case matchStr1 (sign, str, anonSym, tdepth, entEnv, [uncoerced], rpath,
			statenv, region, compInfo)
	  of SOME (resDec, resStr, exp) =>
	       (let val resExp = M.CONSTRAINstr{boundvar=uncoerced, raw=strExp, coercion=exp}
		    val _ = debugmsg "<<matchStr"
		 in SOME {resDec=resDec, resStr=resStr, resExp=resExp}
		end
...
val {resDec=resDec2, resStr=resStr2, resExp=resExp2} =
    let val rp = IP.IPATH[S.strSymbol "<FctResult>"]
     in case matchStr{sign=fsigBodySig, str=resStr1, strExp=resExp1, evOp=NONE,
		      tdepth=tdepth', entEnv=fsigBodySigEnv, rpath=rp,
		      statenv=statenv, region=region, compInfo=compInfo}
	  of SOME x => x
	   | NONE => bogusMatchResult
    end
...
and applyFct{fct as FCT {sign=FSIG{paramsig, bodysig, ...},
			 rlzn = fctRlzn, ... },
             fctExp, argStr, argExp, evOp, epc, tdepth, statenv,
	     rpath, region, compInfo as {mkStamp, mkLvar=mkv, ...}} =
  let val {closure=CLOSURE {env=fctEntEnv, ... }, ... } = fctRlzn
      ...
      val {resDec=argDec1, resStr=argStr1, resExp=argExp1} =
        (case matchStr {sign=paramsig, str=argStr, strExp=argExp, evOp=evOp,
...
| applyFct {fct=ERRORfct, ...} = bogusMatchResult
